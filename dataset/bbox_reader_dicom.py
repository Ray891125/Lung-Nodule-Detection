    ...
    def __getitem__(self, idx):
        ...
        if self.mode in ['train', 'val']:
            if not is_random_img:
                ...
                neg_bboxes = [self.pick_rand_neg(bboxes, imgs.shape) for i in range(max(1, 3-len(pos_bboxes)))]
                
                for pos in pos_bboxes:
                    sample, target, pos_bbox, coord = self.crop(imgs, pos, bboxes, isScale, is_random_crop)               
                    # Data augment
