...
from evaluationScript.new_nodule_finding import NoduleFinding
...
def evaluateCAD(seriesUIDs: List[str], 
    ...
    if PERFORMBOOTSTRAPPING: # True
        ...
        with open(os.path.join(output_dir, "froc_bootstrapping_{}.csv".format(iou_threshold)), 'w') as f:
            ...
            fps2_sens = sens_bs_mean[np.argmin(abs(fps_bs_itp - 2))]
            for fp_point in FPS:
    ...
    return (fps, sens, thresholds, fps_bs_itp, sens_bs_mean, sens_bs_lb, sens_bs_up, sens_points), (fixed_tp, fixed_fp, fixed_fn, fixed_recall, fixed_precision, fixed_f1_score), (best_f1_score, best_f1_threshold),fps2_sens
...
def nodule_evaluation(annot_path: str,
    ...
    out, fixed_out, (best_f1_score, best_f1_threshold),fps2_sens = evaluateCAD(seriesUIDs = seriesUIDs, 
    ...
    return out, fixed_out, (best_f1_score, best_f1_threshold),fps2_sens