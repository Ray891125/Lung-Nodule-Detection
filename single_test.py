...
def main():
    ...
    if args.mode == 'eval':
        ...
        eval(net, test_loader, test_annotation_dir, data_dir, save_dir)
    else:
        logging.error('Mode %s is not supported' % (args.mode))


def eval(net, dataset, test_annotation_dir, data_dir, save_dir=None):
    net.set_mode('eval')
    net.use_rcnn = True
    aps = []
    dices = []  
    ...
    noduleCADEvaluation(test_annotation_dir, data_dir, dataset.dataset.set_name, rpn_submission_path, os.path.join(eval_dir, 'rpn'))

    noduleCADEvaluation(test_annotation_dir, data_dir, dataset.dataset.set_name, rcnn_submission_path, os.path.join(eval_dir, 'rcnn'))

    noduleCADEvaluation(test_annotation_dir, data_dir, dataset.dataset.set_name, ensemble_submission_path, os.path.join(eval_dir, 'ensemble'))
