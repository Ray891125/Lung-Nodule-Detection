...
def box_center_dist_tensor(boxes1: Tensor, boxes2: Tensor, euclidean: bool = True) -> \
        Tuple[Tensor, Tensor, Tensor]:
    """
    Distance of center points between two sets of boxes

    Arguments:
        boxes1: boxes(GT); (z1, y1, x1, d, (h, w))[N, dim * 2]
        boxes2: boxes(Anchor); (z1, y1, x1, d, (h, w))[M, dim * 2]
    ...
    Returns:
        ...
        [[1st gt with 1st anchor, 1st gt with 2nd anchor, ...],[2nd gt with 1st anchor, 2nd gt with 2nd anchor, ...]]
    """
    center1 = boxes1  # [N, dims]
    center2 = boxes2  # [M, dims]

    if euclidean:
        dists = (center1[:, None] - center2[None]).pow(2).sum(-1).sqrt()
        # dists = np.sqrt(np.sum((center1[:, None] - center2[None])**2, axis=-1))
    else:
        # before sum: [N, M, dims]
...
def box_center_dist(boxes1: np.ndarray, boxes2: np.ndarray, euclidean: bool = True) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    Distance of center points between two sets of boxes
    ...
    Returns:
        np.ndarray: the NxM matrix containing the pairwise
            distances for every element in boxes1 and boxes2; [N, M]
        [[1st gt with 1st anchor, 1st gt with 2nd anchor, ...],[2nd gt with 1st anchor, 2nd gt with 2nd anchor, ...]]
    """
    center1 = boxes1  # [N, dims]
    ...
    if euclidean:
        ...
        dists = np.sum(np.abs(center1[:, None] - center2[None]), axis=-1)
    return dists
