...
from net.layer.bbox_ops import box_center_dist
...
def make_one_rpn_target(cfg, mode, input, window, truth_box, truth_label):
    ...
    num_anchors_per_level = [4]
    num_candidates = len(truth_box)
    num_anchors_per_loc = 10
    num_truth_box = len(truth_box)
    num_gt = len(truth_box)
    num_anchors = len(window)

    if num_truth_box:

        _, depth, height, width = input.size()
        distances = box_center_dist(truth_box,window)
        # 1. select candidates based on center distance
        candidate_idx = []
        start_idx = 0
        for level, apl in enumerate(num_anchors_per_level):
            end_idx = start_idx + apl

            selectable_k = min(num_candidates * num_anchors_per_loc, apl)
        ...
        match_quality_matrix = torch_overlap(truth_box,window)  # [num_boxes x anchors]
        candidate_overlaps = match_quality_matrix.gather(1, candidate_idx)  # [num_boxes, n_candidates]
        
        # 2. compute adaptive iou threshold
