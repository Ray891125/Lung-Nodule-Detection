...
import random
from net.layer.ops.NMS.rpn_nms import rpn_encode
from torch.autograd import Variable


def make_one_rpn_target(cfg, mode, input, window, truth_box, truth_label,k=5):
    """
    ...
    label = np.zeros((num_window, ), np.float16)
    label_assign = np.zeros((num_window, ), np.int16) - 1
    label_weight = np.zeros((num_window, ), np.float16)
    target = np.zeros((num_window, 6), np.float16)
    #target_weight = np.zeros((num_window, ), np.float32)
    target_weight = []


    num_truth_box = len(truth_box)
    if num_truth_box and truth_box.sum()!=0:

        _, depth, height, width = input.size()
        #_, _, depth, height, width = input
        ...
        if len(fg_index) >= k:
            idx = random.sample(range(len(fg_index)), k)
        else:
            idx = list(range(len(fg_index)))
        label[fg_index] = 0
