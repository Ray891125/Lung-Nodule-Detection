...
from net.Module.SCconv import MSCConv3D
...
class MSC_ECA(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride = 1,
        ...
        # SCconv3D won't change the channel of input here
        self.MSCconv1 = MSCConv3D(inplanes, planes, stride = stride,
                 cardinality=cardinality, bottleneck_width=bottleneck_width,
                 avd=avd, dilation=dilation, is_first=is_first,
                 norm_layer=norm_layer)
        self.MSCconv2 = MSCConv3D(planes, planes, stride = stride,
                 cardinality=cardinality, bottleneck_width=bottleneck_width,
                 avd=avd, dilation=dilation, is_first=is_first,
                 norm_layer=norm_layer)
        self.eca1 = ECABlock(planes)
...
#         self.MSA3 = MultiScalAggregation2fs([96,120])
    ...
    def forward(self, x):
        ...
        #out4 = self.drop(out4)
#         mid4 = self.MSA3([out3,out4])       # output = 6 * 6 * 6 * 120
#         mid3 = self.MSA2([out2,out3,out4])  # 12* 12* 12* 96
#         mid2 = self.MSA1([out1,out2,out3])  # 24* 24* 24* 72
